...
def generate_image_gallery(image_files):
    ...
    for index, image_path in enumerate(image_files, 1):
        ...
        print(f'✓ Imagen procesada en galería ({label}): {image_path.name}')
...
def format_text_content(text):
    ...
    # Pre-procesamiento: Separar títulos numerados
    text = re.sub(r'(?m)^(\d+\..+)$', r'\n\n\1', text)
    while '\n\n\n' in text:
        text = text.replace('\n\n\n', '\n\n')
    ...
    # Función auxiliar para convertir a Title Case (mayúscula inicial en cada palabra)
    def title_case(text):
    ...
    for para in paragraphs:
        ...
        # Función interna para formatear líneas con el criterio "Negrita solo antes de los dos puntos"
        def bold_labels(line):
            if ':' in line:
                # Si hay dos puntos, negrita solo a la izquierda
                parts = line.split(':', 1)
                return f'<b>{parts[0]}:</b>{parts[1]}'
            else:
                # Si no hay dos puntos, toda la línea en negrita (si es título)
                return f'<b>{line}</b>'

        # Función interna para normalizar mayúsculas excesivas (usar Title Case)
        ...
        if re.match(r'^\d+\.', trimmed) or (trimmed.isupper() and len(trimmed) < 60):
            content = normalize_case(trimmed)
            content = bold_labels(content)
            formatted_html += f'<h3>{content}</h3>\n'
            continue

        # 2. Subtítulos o Reportes (H4) - Ej: Reporte 1: ...
        if trimmed.startswith('REPORTE') or (trimmed.startswith('**') and trimmed.endswith('**')):
            content = normalize_case(trimmed.replace('*', ''))
            content = bold_labels(content)
            formatted_html += f'<h4>{content}</h4>\n'
            continue

        # 3. Párrafos normales
        processed_para = trimmed.replace(chr(10), "<br>")
        
        # Convertir **texto** a <b>texto</b> (markdown manual del usuario)
        processed_para = re.sub(r'\*\*(.*?)\*\*', r'<b>\1</b>', processed_para)
        
        # Aplicar negrita solo antes de los ":" en cada línea del párrafo
        # Y aplicar Title Case a nombres propios después de ciertos labels
        new_lines = []
        for line in processed_para.split('<br>'):
            if ':' in line and not ('<' in line.split(':')[0]): # Evitar romper HTML ya puesto
                parts = line.split(':', 1)
                label = parts[0].strip()
                value = parts[1].strip() if len(parts) > 1 else ''
                
                # Aplicar Title Case a nombres propios
                if any(keyword in label for keyword in ['Local', 'Técnico', 'Responsable']):
        ...
        # Resaltar "Local: ..." en rojo
        if 'Local:' in processed_para:
            processed_para = re.sub(
                r'(<span class="data-label">Local:</span>\s*[^<]+)', 
                r'<span class="local-highlight">\1</span>', 
                processed_para
            )
